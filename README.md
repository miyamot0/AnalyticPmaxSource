...
As first noted by the model authors (Hursh & Roma, 2013; Hursh & Silberberg, 2008), an arithmetic solution for PMAX is not available. The terms of this model’s first order derivative prohibit an arithmetical solution for the price (i.e., 𝑥) at a slope of −1 because the term for price 
...
### Derivative PMAX.
In contrast to numerical approximates, the slope of the Exponential demand curve can be iteratively evaluated using its first order derivative and specialized computer programming. The results of the approach have been previously referred to as a true or Exact PMAX (Hursh & Roma, 2013), as results are determined using a computationally-exhaustive process that directly evaluates model slope at various unit prices until the first order derivative equals −1. The first order derivative, as provided by Hursh and Silberberg (2008), takes the following form:
